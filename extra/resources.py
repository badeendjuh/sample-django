        ...
        try:
            ...
            if method not in self.http_methods.get(endpoint, {}):
                raise MethodNotImplemented(
                        f"Unsupported method '{method}' for {self.endpoint} endpoint"
                )
            ...
            # Ensure this exception goes to email_admins filter WITH a stacktrace
            if not isinstance(err, (BadRequest, Unauthorized, NotFound, PreconditionFailed, Conflict)):
                logger.exception(f'Error encountered when handling method {method} for {self.endpoint}', extra={
